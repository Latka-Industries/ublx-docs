# Guides

Cross-tool workflows for UBLX, Nefaxer, and ZahirScan.

## Available

- [FAQ](/guides/faq) — common questions about the stack
- [Snapshot & browse](/guides/snapshot-and-browse) — index with Nefaxer, browse on the Snapshot tab
- [Path-only vs full enhance](/guides/path-only-vs-full-enhance) — defaults, tradeoffs, when to batch
- [Right-pane previews](/guides/right-pane-previews) — Viewer, Templates, Metadata, Writing
- [Making and exporting lenses](/guides/lenses) — saved path lists and `ublx-lenses/` Markdown
- [Change & duplicates](/guides/change-and-duplicates) — Delta diffs and hash duplicate groups
- [Built for project trees](/guides/project-trees) — large-tree strategy, navigation, what UBLX is for
- [Enhance policies](/guides/enhance-policies) — `auto` vs `manual` per path prefix
- [Headless snapshot + export](/guides/headless-snapshot-export) — CI-friendly `ublx --snapshot-only --export`
- [CLI](/cli) — also `ublx query` / `ublx doctor` / `ublx serve` (and `--url` against serve) for catalog read, diagnosis, and local HTTP
- [Command mode & context menus](/guides/command-mode-and-menus) — Space, Ctrl+Space, Ctrl+leader (default Ctrl+A)
