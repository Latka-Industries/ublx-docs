...
Headless index/export (`--snapshot-only`, `--full-snapshot`, `--export`) and catalog tools (`ublx query`, `ublx doctor`): [CLI](/cli).
...
- [CLI](/cli) — `ublx --help`, headless flags, `query` / `doctor`
